...
console.log('Importing YahooFinance...');
const YahooFinance = require('yahoo-finance2').default;
console.log('Imported. Creating instance...');
let yahooFinance;
try {
...
app.get('/api/stock/:symbol', async (req, res) => {
  ...
  try {
    ...
    // const quote = { symbol, regularMarketPrice: 100, longName: 'Test Stock' };
...
app.post('/api/trade/buy', verifyToken, async (req, res) => {
  ...
  try {
    // Get current price
    const quote = await yahooFinance.quote(symbol);
    // const quote = { symbol, regularMarketPrice: 100 };
...
app.get('/api/portfolio', verifyToken, async (req, res) => {
  ...
  const enriched = await Promise.all(portfolio.map(async (item) => {
    try {
      ...
      // const quote = { symbol: item.symbol, regularMarketPrice: 100 };
      return {
        ...item,
      ...
      return { ...item, current_price: null, day_change: null, day_change_percent: null, profit_loss: null, percent_gain: null };
    }
  }));
